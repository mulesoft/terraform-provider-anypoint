# Import an existing Anypoint Secret Group Certificate into Terraform state.
#
# Steps:
#   1. Replace the placeholder IDs below with your actual values.
#   2. Uncomment the import block and resource block.
#   3. Run: terraform init && terraform apply
#      OR use CLI: terraform import anypoint_secret_group_certificate.imported <import_id>
#
# Import ID format:
#   anypoint_secret_group_certificate -> organization_id/environment_id/secret_group_id/certificate_id
#
# Note: certificate_base64 is a write-only field and will not be populated after import.
...
#   id = "${local.org_id}/${local.env_id}/${local.secret_group_id}/${local.certificate_id}"
# }
...
#   name            = "<name>"
#   type            = "PEM"   # "PEM" or "JKS"
#   # certificate_base64 is write-only; set it to avoid drift after import.
#   # certificate_base64 = base64encode(file("<path/to/cert.pem>"))
# }

# output "imported_id" {
#   value = anypoint_secret_group_certificate.imported.id
# }
