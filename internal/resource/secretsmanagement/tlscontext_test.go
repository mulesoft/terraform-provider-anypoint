...
func TestTLSContextResource_ValidateConfig_CipherSuitesRequiredForTLSv12(t *testing.T) {
	res := NewTLSContextResource().(*TLSContextResource)
	ctx := context.Background()
	...
	buildConfig := func(minTLS string, ciphers []interface{}) resource.ValidateConfigRequest {
		var cipherVal tftypes.Value
		if ciphers == nil {
			cipherVal = tftypes.NewValue(tftypes.List{ElementType: tftypes.String}, nil)
		} else {
			elems := make([]tftypes.Value, len(ciphers))
			for i, c := range ciphers {
				elems[i] = tftypes.NewValue(tftypes.String, c)
			}
			cipherVal = tftypes.NewValue(tftypes.List{ElementType: tftypes.String}, elems)
		}
		raw := tftypes.NewValue(stateType, map[string]tftypes.Value{
			"id":                           tftypes.NewValue(tftypes.String, nil),
			"organization_id":              tftypes.NewValue(tftypes.String, nil),
			"environment_id":               tftypes.NewValue(tftypes.String, "env-1"),
			"secret_group_id":              tftypes.NewValue(tftypes.String, "sg-1"),
			"name":                         tftypes.NewValue(tftypes.String, "test"),
			"target":                       tftypes.NewValue(tftypes.String, "OmniGateway"),
			"keystore_id":                  tftypes.NewValue(tftypes.String, nil),
			"truststore_id":                tftypes.NewValue(tftypes.String, nil),
			"min_tls_version":              tftypes.NewValue(tftypes.String, minTLS),
			"max_tls_version":              tftypes.NewValue(tftypes.String, "TLSv1.3"),
			"alpn_protocols":               tftypes.NewValue(tftypes.List{ElementType: tftypes.String}, nil),
			"cipher_suites":                cipherVal,
			"enable_client_cert_validation": tftypes.NewValue(tftypes.Bool, false),
			"skip_server_cert_validation":  tftypes.NewValue(tftypes.Bool, false),
			"expiration_date":              tftypes.NewValue(tftypes.String, nil),
		})
	...
	t.Run("TLSv1.2 with no cipher_suites errors", func(t *testing.T) {
		req := buildConfig("TLSv1.2", nil)
		resp := &resource.ValidateConfigResponse{}
		res.ValidateConfig(ctx, req, resp)
		if !resp.Diagnostics.HasError() {
			t.Error("expected error when min_tls_version=TLSv1.2 and cipher_suites is empty")
		}
	})

	t.Run("TLSv1.2 with cipher_suites set passes", func(t *testing.T) {
		req := buildConfig("TLSv1.2", []interface{}{"TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256"})
		resp := &resource.ValidateConfigResponse{}
		res.ValidateConfig(ctx, req, resp)
		if resp.Diagnostics.HasError() {
	...
	t.Run("TLSv1.3 with no cipher_suites passes", func(t *testing.T) {
		req := buildConfig("TLSv1.3", nil)
		resp := &resource.ValidateConfigResponse{}
		res.ValidateConfig(ctx, req, resp)
		if resp.Diagnostics.HasError() {
			t.Errorf("unexpected error for TLSv1.3 without cipher_suites: %v", resp.Diagnostics.Errors())
		}
	})
}
