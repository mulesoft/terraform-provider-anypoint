...
func TestNewTeamsDataSource(t *testing.T) {
	dataSource := NewTeamsDataSource()

	if dataSource == nil {
		t.Error("NewTeamsDataSource() returned nil")
	}

	if _, ok := dataSource.(datasource.DataSourceWithConfigure); !ok {
		t.Error("TeamsDataSource does not implement DataSourceWithConfigure")
	}
}
...
func TestTeamsDataSource_Metadata(t *testing.T) {
	...
	if resp.TypeName != "test_teams" {
		t.Errorf("Metadata() TypeName = %v, want %v", resp.TypeName, "test_teams")
	}
}
...
func TestTeamsDataSource_Schema(t *testing.T) {
	...
	optionalAttrs := []string{"name_filter"}
...
func TestTeamsDataSource_Configure(t *testing.T) {
	dataSource := NewTeamsDataSource().(*TeamsDataSource)
...
func BenchmarkTeamsDataSource_Schema(b *testing.B) {
	dataSource := NewTeamsDataSource()
	ctx := context.Background()
	req := datasource.SchemaRequest{}
