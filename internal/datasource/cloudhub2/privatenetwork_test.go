...
func TestNewPrivateNetworkDataSource(t *testing.T) {
	dataSource := NewPrivateNetworkDataSource()

	if dataSource == nil {
		t.Error("NewPrivateNetworkDataSource() returned nil")
	}
...
func TestPrivateNetworkDataSource_Metadata(t *testing.T) {
	dataSource := NewPrivateNetworkDataSource()
	...
	if resp.TypeName != "test_private_network" {
		t.Errorf("Metadata() TypeName = %v, want %v", resp.TypeName, "test_private_network")
	}
}

func TestPrivateNetworkDataSource_Schema(t *testing.T) {
	dataSource := NewPrivateNetworkDataSource()
	...
	computedAttrs := []string{"name", "region", "status", "created_at", "updated_at"}
...
func TestPrivateNetworkDataSource_Configure(t *testing.T) {
	dataSource := NewPrivateNetworkDataSource().(*PrivateNetworkDataSource)
...
func TestPrivateNetworkDataSourceModel_Validation(t *testing.T) {
	model := PrivateNetworkDataSourceModel{}
	_ = model.ID
}

func BenchmarkPrivateNetworkDataSource_Schema(b *testing.B) {
	dataSource := NewPrivateNetworkDataSource()
	ctx := context.Background()
	req := datasource.SchemaRequest{}
