---
page_title: "anypoint_api_policy_mcp_pii_detector Data Source - terraform-provider-anypoint"
subcategory: "API Policies"
description: |-
  Reads the MCP PII Detector policy applied to an Anypoint API instance.
---

# anypoint_api_policy_mcp_pii_detector (Data Source)

Reads the MCP PII Detector policy applied to an Anypoint API instance. If `policy_id` is omitted, the data source automatically finds the policy by its asset type on the given API instance.
...
output "mcp_pii_detector_configuration" {
  value = data.anypoint_api_policy_mcp_pii_detector.example.configuration
}

# Or look up by explicit policy_id
data "anypoint_api_policy_mcp_pii_detector" "by_id" {
  organization_id = var.organization_id
  environment_id  = var.environment_id
  api_instance_id = var.api_instance_id
  policy_id       = "12345"
