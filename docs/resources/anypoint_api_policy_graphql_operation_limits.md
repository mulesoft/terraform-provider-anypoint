---
page_title: "anypoint_api_policy_graphql_operation_limits Resource - terraform-provider-anypoint"
subcategory: "API Policies"
description: |-
  Manages a GraphQL Operation Limits policy on an Anypoint API instance.
---

# anypoint_api_policy_graphql_operation_limits (Resource)

Manages a GraphQL Operation Limits policy on an Anypoint API instance. This policy enforces limits on GraphQL operation characteristics such as query depth, number of aliases, root fields, and directives.
...
```terraform
resource "anypoint_api_policy_graphql_operation_limits" "example" {
...
# Disable all limits (use -1 for unlimited)
resource "anypoint_api_policy_graphql_operation_limits" "unlimited" {
  organization_id = var.organization_id
  environment_id  = var.environment_id
  api_instance_id = anypoint_api_instance.example.id
...
- `max_depth` (Number) Maximum allowed depth of GraphQL query nesting. Set to `-1` for unlimited. Defaults to `-1`.
- `max_aliases` (Number) Maximum number of aliases allowed in a GraphQL operation. Set to `-1` for unlimited. Defaults to `-1`.
- `max_root_fields` (Number) Maximum number of root-level fields allowed in a GraphQL operation. Set to `-1` for unlimited. Defaults to `-1`.
- `max_directives` (Number) Maximum number of directives allowed in a GraphQL operation. Set to `-1` for unlimited. Defaults to `-1`.
...
An existing `anypoint_api_policy_graphql_operation_limits` policy can be imported using its composite ID: `organization_id/environment_id/api_instance_id/policy_id`.
...
```terraform
import {
  to = anypoint_api_policy_graphql_operation_limits.imported
  id = "<organization_id>/<environment_id>/<api_instance_id>/<policy_id>"
}

resource "anypoint_api_policy_graphql_operation_limits" "imported" {
...
```shell
terraform import anypoint_api_policy_graphql_operation_limits.imported <organization_id>/<environment_id>/<api_instance_id>/<policy_id>
```
