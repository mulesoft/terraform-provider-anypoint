...
-> **Immutable versions:** `group_id`, `asset_id`, `version`, `type`, `classifier`, `file_path`, `api_version`, `main_file`, and the `additional_file` set are replacement-forcing. Changing any of them destroys the existing asset version and creates a new one. (On **import** the file-content fields — `file_path`, `classifier`, `api_version`, `main_file`, `additional_file` — are seeded from state and do not trigger a spurious replace.) To publish an additional version side-by-side (like the Exchange UI "Add version" button), add a separate resource block rather than editing `version` in place.
