...
-> **Immutable versions:** `version`, `type`, `classifier`, and `file_path` are replacement-forcing. Changing any of them destroys the existing asset version and creates a new one. To publish an additional version side-by-side (like the Exchange UI "Add version" button), add a separate resource block rather than editing `version` in place.

~> **Versions cannot be reused — bump, don't reuse.** Exchange **soft-deletes** asset versions: once a version (GAV) has been published, that version number is **reserved permanently**. Deleting the asset version (or a Terraform destroy→recreate) does **not** free it — re-publishing the same `group/asset/version` fails with `409 ASSET_PRE_CONDITIONS_FAILED` ("an asset already exists with this version"). This is why an in-place change to `classifier`/`file_path`/`tags` at the **same** `version` cannot succeed even though it forces a replace: the recreate republishes the same, still-reserved version. Always **bump `version`** to publish new content. The provider detects this 409 and returns an actionable "version already exists; bump the version" error instead of the raw platform code.
