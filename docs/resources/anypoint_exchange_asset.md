...
-> **Immutable versions:** `organization_id`, `group_id`, `asset_id`, `version`, `type`, `classifier`, `file_path`, `api_version`, `main_file`, `keywords`, and the `additional_file` set are replacement-forcing. Changing any of them destroys the existing asset version and creates a new one. **Removing** one of the optional ones (`file_path`, `keywords`, `additional_file`) from your configuration counts as a change and replaces the version too — deleting the attribute is not the same as leaving it unmanaged. (On **import** the file-content fields — `file_path`, `classifier`, `api_version`, `main_file`, `additional_file` — are seeded from state and do not trigger a spurious replace.) To publish an additional version side-by-side (like the Exchange UI "Add version" button), add a separate resource block rather than editing `version` in place.

~> **`keywords` is replacement-forcing, which surprises people.** It reads like ordinary searchable metadata, but Exchange stores it on the immutable version, so editing or deleting a keyword hard-deletes the published version and republishes it. Bump `version` in the same apply if you need to change keywords on a live asset.

~> **Publish new content by bumping `version` — don't edit a published version in place.** Published content is immutable, so changing any content field (`file_path`, `type`, `classifier`, `api_version`, `main_file`, `additional_file`) forces a **replacement** (destroy + create) rather than an in-place update. The provider issues a **hard delete** (`x-delete-type: hard-delete`) on destroy, and a same-version replacement destroys the current version *before* republishing it — so it cannot use `create_before_destroy` (see below) and a failed republish can leave you with nothing. Bumping `version` avoids that risk entirely (the new GAV coexists with the old). If you try to **publish onto a version that already exists** and is not being replaced by this apply — a fresh create whose `group/asset/version` is already published, or a bump onto an already-taken number — Exchange rejects it with `409 ASSET_PRE_CONDITIONS_FAILED`; the provider catches this **at plan time** (before any destroy) and again at apply with an actionable "bump the version" message. Note that `status` and `tags` are **mutable in place** and never require a republish.
...
- `keywords` (String) Comma-separated keywords for search discovery. **Replacement-forcing:** stored on the immutable asset version, so changing or removing this destroys and republishes the version — see [Immutable versions](#anypoint_exchange_asset-resource) above.
