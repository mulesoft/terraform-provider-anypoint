---
page_title: "anypoint_api_policy_websocket_connection_limit Resource - terraform-provider-anypoint"
subcategory: "API Policies"
description: |-
  Manages a WebSocket Connection Limit policy on an Anypoint API instance.
---

# anypoint_api_policy_websocket_connection_limit (Resource)

Manages a WebSocket Connection Limit policy on an Anypoint API instance. This policy limits the number of concurrent WebSocket connections to prevent resource exhaustion.
...
```terraform
resource "anypoint_api_policy_websocket_connection_limit" "example" {
...
# High-capacity WebSocket service
resource "anypoint_api_policy_websocket_connection_limit" "high_capacity" {
  organization_id = var.organization_id
  environment_id  = var.environment_id
  api_instance_id = anypoint_api_instance.example.id

  configuration = {
    maximum_connections = 5000
  }
...
- `maximum_connections` (Number) Maximum number of concurrent WebSocket connections allowed. Defaults to `100`.
...
An existing `anypoint_api_policy_websocket_connection_limit` policy can be imported using its composite ID: `organization_id/environment_id/api_instance_id/policy_id`.
...
```terraform
import {
  to = anypoint_api_policy_websocket_connection_limit.imported
  id = "<organization_id>/<environment_id>/<api_instance_id>/<policy_id>"
}

resource "anypoint_api_policy_websocket_connection_limit" "imported" {
...
```shell
terraform import anypoint_api_policy_websocket_connection_limit.imported <organization_id>/<environment_id>/<api_instance_id>/<policy_id>
```
