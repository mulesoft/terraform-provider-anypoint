...
| `anypoint_apim_flexgateway` | `anypoint_managed_omni_gateway` (plus `anypoint_api_instance` for the API side) | **Renamed + Schema change** | Managed Omni Gateway infrastructure is a first-class resource now; API-instance attachment goes through `anypoint_api_instance`. |
...
`anypoint_secret_group_*` (plural variants), `anypoint_managed_omni_gateway`,
